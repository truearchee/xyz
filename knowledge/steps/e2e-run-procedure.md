...
#    (some specs read SUPABASE_SERVICE_ROLE_KEY/E2E_TEST_PASSWORD from process.env), and target host :3000.
set -a; . ./.env.e2e; set +a
PLAYWRIGHT_BASE_URL=http://localhost:3000 npx playwright test --workers=1   # --workers=1 required (4.7-R2)
...
- **Run on host `:3000`** — **FIXED in 12f (Commit 1, F-12C-CORS).** The committed `docker-compose.yml` now
  maps the frontend `"3000:3000"` (canonical) and `.env.example` ships
  `CORS_ORIGINS=http://localhost:3000,http://localhost:3001`, so a fresh `.env` accepts the preflight on
  either port. Historically (pre-12f) the frontend mapped `"3001:3000"` with a `:3000`-only CORS default,
  which rejected the `:3001` preflight (`OPTIONS /me` → 400) and bounced every login to `/login`. The
  `tests/e2e/12f-cors-preflight.spec.ts` assertion now pins this so it cannot silently regress.
