...
## Current structure
The backend auth boundary is `backend/app/platform/auth/`. JWT parsing and verification live in `jwt.py`, request identity resolution lives in `dependencies.py`, and immutable request identity types live in `context.py`.
...
## Route boundary
Routes consume `Depends(get_current_user)` and do not parse JWTs or query app users directly. The only route added in this session is `GET /health/authed`, a smoke endpoint for auth wiring.
