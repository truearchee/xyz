...
_Last updated: 2026-06-13 - **Stage 4.9 (frontend foundation + platform hygiene) FULLY VERIFIED — automated evidence all green; 3 developer-owned residuals stated, not hidden.** The thin inline-styled frontend is converted onto a styling system + component library + test/CI harness with NO new product feature. Delivered across sub-sessions: **4.9a** Tailwind v4 (`@tailwindcss/postcss`) + two-layer tokens (raw `--palette-*` → semantic `@theme`; default colour namespace disabled, build-enforced) + next/font/local (Space Grotesk + Inter, offline; mono dropped) + shell infra (AppShell, error/not-found/unauthorized, toast/modal portal roots, z-layer tokens); **4.9b** 9-component library (Button/Input/Card/Badge/Modal[React Aria]/Table/Toast[owned]/EmptyState/Progress+Step signature pipeline) + helpers, literal-class variant maps, a11y contracts; **4.9c** bounded retroactive restyle (login/admin/lecturer/student/shell) — inline styles removed, every `data-testid`/`__xyzE2E` selector preserved; **4.9d** Vitest harness (13 logic + 11 a11y tests), §8 gate scripts, `.github/workflows/ci.yml`, Husky pre-commit (PROVEN blocking), keyboard checklist, ADR-044..048; **4.9e** hygiene batch + close-out. **4.9e:** §7.1 httpx→`ASGITransport` across **3** test files (conftest + test_health + test_sse_probe) — `pytest -W error::DeprecationWarning` gives **421 passed, 0 deprecations** (first gate caught conftest-only left 6 warnings — fixed); §7.2 dropped CORS `allow_credentials` (pure Bearer; cross-origin E2E + test_health CORS tests green); §7.3 `gen:api` alias; §7.4 E2E data-hygiene PREVENTION (F-4.9-2) — `check-orphans.mjs` pre-run fail-loud + `run-active-suite.sh` `trap EXIT` unconditional teardown, **DEMONSTRATED** (clean→ok / injected→exit1 / cleared→ok). Close-out: contrast **35/35** (incl. `--color-overlay` composited as a scrim 3.94:1 after 45→55%, + `--color-danger-hover`); **`design-system.md` written FROM shipped code** (v1.0.0, large/UI-only solid-status rule); **design-match evidence** — 16 desktop+mobile screenshots, **mobile sanity 8/8 surfaces 0px at 375px**. **F-4.9-6** found+fixed (mobile overflow on desktop-first /admin 150px + /lecturer 5px; root-caused by a DOM ancestor-chain trace = grid `min-width:auto` + `<select>` intrinsic width, NOT tables; CSS-only `min-w-0`/`[&>*]:min-w-0`/`w-full`; desktop 2-col preserved). **Full active Playwright suite 11/11** (CORS off, selectors intact) on the image with all CSS fixes; all 7 §8 gates green; `build` compiles the arbitrary variants. ADR-049. Findings: F-4.9-1/-2/-6 RESOLVED, F-4.9-3 accepted-w-rationale (Next/postcss CVEs unreachable → Stage 12), F-4.9-4 deferred Stage 12 (dead `features/content/*`), F-4.9-5 deferred (remote branch-protection, owner: developer, trigger: next push). **3 developer-owned residuals (roadmap footnote):** design-match visual sign-off (evidence captured), keyboard pass (checklist authored), remote-CI enforcement (F-4.9-5). **NOT closed by 4.9 (stated):** backend mypy/pyright (frontend tsc only), E2E `--workers=1` (4.7-R2) → both Stage 12. Next → Stage 5 (or the hosted 4.8 gate)._

_Prior: 2026-06-12 - **Stage 4.9 (frontend foundation + platform hygiene) STARTED — Phase 0 complete; pre-build pause.** Gated-workflow Phase 0 done: (1) **Design direction LOCKED** in a facilitated session — **Violet / AI-forward** palette (primary `#7c3aed`), **Space Grotesk + Inter + JetBrains Mono**, signature = **the processing pipeline** (bold step track w/ a striking failed state); WCAG **AA**; student surfaces mobile-first. `knowledge/design-plan.md` **created** (it did not previously exist anywhere — the hard blocker on prereq 1a/1b/2). (2) **Umbrella spec filed** (`specs/stage-04/4.9-frontend-foundation-platform-hygiene.md`, verbatim + filing note reconciling all 5 prereqs). (3) **Prereq 4 baseline GREEN 11/11** at SHA `6dd12db` via the documented 3-part orchestration (9 success `--grep-invert "fault gate"` + invalid_output + invalid_input, each fault spec under the `ai_worker` `LLM_FAULT_INJECTION` overlay) — `steps/stage-04/4.9-baseline.md`. **F-4.9-1 (= 4.8's deferred F-4.8d-1) RESOLVED:** purged **58 orphaned e2e `course_modules`** (admin list `?limit=50` vs accumulation; create returned **201** → NOT a code regression), kept 2 dev modules. **Decisions:** proceed on 4.8 **BACKEND VERIFIED** (accept-with-rationale, prereq 0); CI = **GitHub Actions required checks + Husky pre-commit** (prereq 5). **F-4.9-2 OPEN** (admin-list flake root cause, to close during build). **No source code changed yet** (only knowledge docs). ADRs adr-044..049 reserved. Master plan: `/Users/arthur.leontev/.claude/plans/stage-4-9-sharded-muffin.md`. Next → **4.9a** (Tailwind v4 + two-layer tokens + shell infra), after developer review of Phase 0._

_Prior: 2026-06-12 - **Stage 4.8 (first hosted deploy / staging) BACKEND VERIFIED — hosted browser gate DEFERRED.** 4.8a–d all BUILT + locally verified: backend `pytest` **421 passed**; full active Playwright suite **11/11** (4.8a–d injected; 9 success + 2 fault); MF1 release-abort proven 3 ways (set-e propagation + real poison revision + live `release.sh`); §8 prod bundle **byte-clean** of `__xyzE2E` (webpack exclude, binds to Dockerfile.prod); SSE probe (3–5 chunks, anti-buffering, admin-only, gated); `/health/ready` 200; `check-staging-env` HARD deploy gate; hook-free staging smoke (NC1≡NC2 byte-equality + NC4 three-surface + machine-written artifact JSON) built + parse-checked. ADRs adr-040..043. The **HOSTED browser gate** (pooler pair → NC1≡NC2 → NC4 three-surface → artifact JSON + the real K2Think rule-11 model-ID echoes) is **DEFERRED** — owner: developer; trigger: before Stage 8.3 or any demo / real-user exposure. **Stage 4.6 + 4.7 FULLY VERIFIED** (gates: 4.6d replace+retry, 4.7 student-summaries). On `stage/4.8-staging-deploy` → merged to main. Next → 4.9 (frontend foundation + hygiene) or the hosted 4.8 gate._
...
## Done recently
- 2026-06-13: **Post-4.9 corrective — real summaries working end-to-end on the real transcript.** Workstream B (inline student summaries, `34e656f`) + Workstream A (real K2Think): real key → `K2-Think-v2` live-verified (F-4.5-27 closed). The real Checkpoint A lecture (~47KB/~11.6K tok) then **408'd on BOTH routes** (single-call size ceiling — the first smoke only proved a ~150-tok fixture; F-4.5-50). Fixed via **Option A (truncate input to 12000 chars, LABELED `truncated` on the record + inline frame, promptVersion v2; ADR-050)**: both Checkpoint A lectures regenerate real, grounded (functions/limits), validator-conformant, **no 408s**; AIRequestLog real (HTTP 200, 26–88s, chatcmpl- ids). Side-fixes: queues.py RQ `job_timeout` > HTTP timeout (was killing long calls); §3A moved to a gate-only `docker-compose.deterministic.yml` + a `run-active-suite.sh` guard (dev=k2think, gate=deterministic). Full-coverage map-reduce = **F-4.5-51** (own spec, out of 4.5). migration 0014. **RESIDUAL: rotate the exposed key** (MBZUAI).
- 2026-06-10: Comprehensive knowledge base review completed; report at `knowledge/KNOWLEDGE_REVIEW.md` (relocated 2026-06-13 from repo root to sit with the cross-cutting docs). Every file in `knowledge/` was read; 27 anomalies identified. **All 27 RESOLVED 2026-06-13** (rule-13 states in the report): ~21 fixed (statuses/hashes/worker.md/log-types/paths — most pre-made, verified + committed), F-001..3 + P-001 + types/frontmatter accepted-with-rationale (convention canonicalized in dev-workflow vs churning 80+ files), C-001/Q-001 fixed.
