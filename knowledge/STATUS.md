...
## Done recently
- 2026-06-13: **Post-4.9 corrective CLOSED.** Workstream B (inline student summaries, `34e656f`) + Workstream A (real K2Think provider): real key → `K2-Think-v2` live-verified end-to-end on both routes (cerebras brief + nvidia detailed, HTTP 200, real grounded summaries, validator-conformant) — **F-4.5-27 RESOLVED**; no code change (model already K2-Think-v2 per ADR-025); `4.5-real-provider-smoke.md`. Dev `.env`=k2think; E2E/CI forced `deterministic` in `docker-compose.e2e.yml` (rule 11 / §3A). **RESIDUAL: rotate the exposed key** (MBZUAI). The corrective started from "summaries show generic stubs" → confirmed it was the deterministic adapter, not a defect.
